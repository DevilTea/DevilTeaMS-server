...
function startBonus(eim) {
	var bonusTime = 60 * 1000;//1 Minute
	var bonusMap = eim.getMapInstance(922011000);
	var party = eim.getPlayers();
	
	eim.cancelAllSchedules();
	eim.schedule("finishBonus", bonusTime); // invokes "timeOut" in how ever many seconds.
	eim.startEventTimer(bonusTime);
