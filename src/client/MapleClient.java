...
public class MapleClient {
	...
	private String hwid = null;
        ...
        private AutoUnstuck autoUnstuck;
	...
	public boolean checkPic(String other) {
		...
		loginattempt++;
                if(getAutoRegister() != null) loginattempt = 0;
                if(getAutoUnstuck() != null) loginattempt = 0;
		if (loginattempt > 4) {
			getSession().close();
		}
		int loginok = 5;
	...
	public final void disconnect(boolean shutdown, boolean cashshop) {//once per MapleClient instance
            ...
            if (player != null && player.isLoggedin() && player.getClient() != null) {
                ...
                player.saveToDB();
                final WorldServer worlda = getWorldServer();
                try {
                    if (!cashshop) {
            ...
            if (!serverTransition && isLoggedIn()) {
                ...
                connection.attr(MapleClient.CLIENT_KEY).set(null); // prevents double dcing during login
                connection.close();
            }
            engines.clear();
	}
	...
	public boolean deleteCharacter(int cid) {
		...
		try {
			...
			String[] toDel = {"famelog", "inventoryitems", "keymap", "queststatus", "savedlocations", "skillmacros", "skills", "eventstats", "taskrecord"};
			for (String s : toDel) {
                            if(s.equals("inventoryitems")) {
                                PreparedStatement ps = con.prepareStatement("SELECT inventoryitemid FROM inventoryitems WHERE characterid = ?");
                                PreparedStatement ps2;
                                ps.setInt(1, cid);
                                ResultSet rs = ps.executeQuery();
                                while(rs.next()) {
                                    ps2 = con.prepareStatement("DELETE FROM inventoryequipment WHERE inventoryitemid = ?");
	...
	public void removeClickedNPC(){
       ...
       public AutoUnstuck getAutoUnstuck() {
           return autoUnstuck;
       }
       
       public void setAutoUnstuck(AutoUnstuck au) {
           autoUnstuck = au;
       }
       
       public TaskRecord getTaskRecord(int mode) {//mode 0 : Task for account ; mode 1 : Task for character
           return TaskRecord.getInstance(loggedIn ? this.accId : -1, (this.player != null) ? this.player.getId() : -1, mode);
       }
}
