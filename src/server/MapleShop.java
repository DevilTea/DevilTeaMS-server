...
public class MapleShop {
    ...
    static {
        ...
        rechargeableItems.remove(2070022); // doesn't exist
    ...
    public void sendShop(MapleClient c) {
        ...
        c.announce(MaplePacketCreator.serverNotice(1, " ~ 歡迎光臨 ~ "));
    }
        ...
        if (item != null && item.getPrice() > 0) {
            if (c.getPlayer().getMeso() >= (long) item.getPrice() * quantity) {
                if (MapleInventoryManipulator.checkSpace(c, itemId, quantity, "")) {
                    ...
                    c.announce(MaplePacketCreator.serverNotice(1, "你沒有足夠的背包空間"));
                
            } else {
                c.announce(MaplePacketCreator.shopTransaction((byte) 0x8));
                c.announce(MaplePacketCreator.serverNotice(1, "你沒有足夠的楓幣"));
            }
    ...
    public void sell(MapleClient c, MapleInventoryType type, short slot, short quantity) {
        ...
        if (quantity <= iQuant && iQuant > 0) {
            ...
            c.announce(MaplePacketCreator.shopTransaction((byte) 0x8));
            c.announce(MaplePacketCreator.serverNotice(1, "販賣獲得 " + recvMesos + " 楓幣"));
        }
    }
