...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    public void giveExpToCharacter(MapleCharacter attacker, int exp, boolean isKiller, int numExpSharers) {
        ...
        final int partyModifier = numExpSharers > 1 ? ((500 + (100 * (numExpSharers - 2))) < 1000 ? (500 + (100 * (numExpSharers - 2))) : 1000) : 0;
        //base 2 people partyexp = 0.5*; extra 1 member + 0.1*; max will be 1*
        int partyExp = 0;
        ...
        if (attacker.getHp() > 0) {
            ...
            if (exp > 0) {
                ...
                Integer expbuff = attacker.getBuffedValue(MapleBuffStat.EXPRATE);
                if (expbuff != null) {
                    personalExp *= (1.0 + (expbuff.doubleValue() / 100.0));
                }
            ...
            if(isKiller) attacker.increaseEquipExp(this.getExp());
        }
    }
    ...
    public void changeHp(long newHp) {
        newHp = Long.min((long) Integer.MAX_VALUE, newHp);
        this.stats.setHp((int) newHp);
        this.hp = (int) newHp;
    }
    
    public void changeMp(long newMp) {
        newMp = Long.min((long) Integer.MAX_VALUE, newMp);
        this.stats.setMp((int) newMp);
        this.mp = (int) newMp;
    }
    
    public void changeExp(long newExp) {
        newExp = Long.min((long) Integer.MAX_VALUE, newExp);
        this.stats.setExp((int) newExp);
    }
}
