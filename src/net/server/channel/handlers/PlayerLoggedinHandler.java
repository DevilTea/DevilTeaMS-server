...
public final class PlayerLoggedinHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (state != MapleClient.LOGIN_SERVER_TRANSITION || !allowLogin) {
            ...
            try {
                ps = con.prepareStatement("UPDATE accounts SET loggedin = 2 WHERE id = ?");
                ps.setInt(1, aid);
                ps.executeUpdate();
            } catch (SQLException e) {
                e.printStackTrace();
        ...
        WorldServer world = server.getWorld(c.getWorld());
        ...
        if (newcomer){
            player.announce(MaplePacketCreator.earnTitleMessage("▁▂▃▄▅▆▇ 歡迎來到DevilTeaMS ▇▆▅▄▃▂▁"));
        }
    }
}
