...
public class MapleGuild {
    ...
    public void buildNotifications() {
        ...
        synchronized (members) {
            for (MapleGuildCharacter mgc : members) {
                ...
                //Unable to connect to ChannelServer... error was here
            }
        }
        bDirty = false;
    }
    ...
    public void guildMessage(final byte[] serverNotice) {
        for (MapleGuildCharacter mgc : members) {
            for (ChannelServer cs : Server.getInstance().getChannelsFromWorld(world)) {
    ...
    public void guildChat(String name, int cid, String message) {
        ...
        String namefrom = Server.getInstance().getWorld(world).getPlayerStorage().getCharacterById(cid).getName();
        this.broadcast(MaplePacketCreator.serverNotice(6, "(公會) " + namefrom + " : " + message), cid);
    }
