...
public class Server implements Runnable {
    ...
    private List<WorldServer> worlds = new ArrayList<>();
    ...
    public void removeChannel(int worldid, int channel) {
        ...
        WorldServer world = worlds.get(worldid);
        if (world != null) {
    ...
    public void run() {
        ...
        try {
            p.load(new BufferedReader(new InputStreamReader(new FileInputStream("world.ini"), "UTF-8")));
        } catch (Exception e) {
            System.out.println("Please start create_server.bat");
            System.exit(0);
        }

        System.out.println("//====================\\\\\r\n||啟動 DevilTeaMS v." + ServerConstants.VERSION + "||\r\n\\\\====================//\r\n");
        ...
        System.out.print("[加載技能中].....");
        long timeToTake = System.currentTimeMillis();
        SkillFactory.loadAllSkills();
        System.out.println("[技能加載完成].....經過 " + ((System.currentTimeMillis() - timeToTake) / 1000.0) + " 秒\r\n");

        System.out.print("[加載道具中].....");
        timeToTake = System.currentTimeMillis();
        MapleItemInformationProvider.getInstance().getAllItems();
        CashItemFactory.getSpecialCashItems();
        System.out.println("[道具加載完成].....經過 " + ((System.currentTimeMillis() - timeToTake) / 1000.0) + " 秒\r\n");
        
        System.out.print("[加載任務中].....");
        timeToTake = System.currentTimeMillis();
        MapleQuest.loadAllQuest();
        System.out.println("[任務加載完成].....經過 " + ((System.currentTimeMillis() - timeToTake) / 1000.0) + " 秒\r\n");
		
		
        try {
            int exprate=0,droprate=0,mesorate=0,bossdroprate=0,mobspawnrate=0;
            for (int i = 0; i < Integer.parseInt(p.getProperty("worlds")); i++) {
                System.out.println("===============[啟動 世界 " + i + "]===============");
                exprate=ServerConstants.EXP_RATE * Integer.parseInt(p.getProperty("exprate" + i));
                droprate=ServerConstants.DROP_RATE * Integer.parseInt(p.getProperty("droprate" + i));
                mesorate=ServerConstants.MESO_RATE * Integer.parseInt(p.getProperty("mesorate" + i));
                bossdroprate=ServerConstants.BOSS_DROP_RATE * Integer.parseInt(p.getProperty("bossdroprate" + i));
                mobspawnrate=ServerConstants.MOB_SPAWN_RATE * Integer.parseInt(p.getProperty("mobspawnrate" + i));
                        
                WorldServer world = new WorldServer(i,Integer.parseInt(p.getProperty("flag" + i)),p.getProperty("eventmessage" + i),exprate,droprate,mesorate,bossdroprate,mobspawnrate);
                System.out.println("    [經驗倍率] : " + exprate + "x\r\n    [掉寶倍率] : " + droprate + "x\r\n    [楓幣倍率] : " + mesorate + "x\r\n    [怪物數量倍率] : " + mobspawnrate + "x\r\n");
        ...
        init = new ServerConnection(8484);
        init.run();
        System.out.println("[登入伺服器] Listening on port 8484\r\n\r\n");
        System.out.println("DevilTeaMS 啟動完成 !");
        online = true;
        canlogin = true;
        transforming = false;
        Console.start();
    ...
    public void reloadGuildCharacters(int world) {
        WorldServer worlda = getWorld(world);
    ...
    public void broadcastMessage(final byte[] packet) {
        for(WorldServer wd : worlds) {
            wd.broadcastPacket(packet);
        }
    }

    public void broadcastGMMessage(final byte[] packet) {
        for(WorldServer wd : worlds) {
            wd.broadcastGMPacket(packet);
        }
    }
    ...
    public final Runnable shutdown(final boolean restart) {//only once :D
        return new Runnable() {
            ...
            public void run() {
                ...
                System.out.println("正在" + (restart ? "重新啟動" : "關閉") + "伺服器 !\r\n");
                for (WorldServer w : getWorlds()) {
                    w.shutdown();
                }
                /*for (WorldServer w : getWorlds()) {
