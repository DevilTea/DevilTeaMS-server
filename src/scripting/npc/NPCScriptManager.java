    ...
    public void start(MapleClient c, int npc, String fileName, MapleCharacter chr) {
        try {
            ...
            if (c.canClickNPC()) {
                ...
                if (iv == null || NPCScriptManager.getInstance() == null) {
                    if(c.getPlayer().isGM()) c.getSession().write(MaplePacketCreator.serverNotice(5, "『系統提醒』NPC : " + npc + " 沒有腳本"));
                    dispose(c);
                    return;
                }
                if(c.getPlayer().isGM()) {
                    if (fileName != null) {
                        c.getSession().write(MaplePacketCreator.serverNotice(5, "『系統提醒』執行了NPC : " + npc + " 腳本 : " + fileName));
                    } else {
                        c.getSession().write(MaplePacketCreator.serverNotice(5, "『系統提醒』執行了NPC : " + npc));
                    }
                }
                engine.put("cm", cm);
                scripts.put(c, iv);
