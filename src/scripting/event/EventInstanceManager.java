...
public class EventInstanceManager {
    ...
    public void removeStopSchedule() {
        for(int i = 0;i < schedules.size();i++) {
            if(schedules.get(i).isCancelled() || schedules.get(i).isDone()) {
                schedules.remove(schedules.get(i));
                i = 0;
            }
        }
    }
    ...
    public ScheduledFuture<?> schedule(final String methodName, long delay) {
        removeStopSchedule();
        ScheduledFuture<?> s = (TimerManager.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                try {
    ...
    public void cancelAllSchedules() {
        for(ScheduledFuture<?> s : schedules) {
            s.cancel(true);
        }
        schedules.clear();
    }
    ...
    public void setVariable(String vname, Object value) {
        if(variables.get(vname) != null) variables.remove(vname);
